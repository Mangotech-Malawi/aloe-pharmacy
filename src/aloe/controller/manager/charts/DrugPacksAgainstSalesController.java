 ...
 * @author Senze
 */
public class DrugPacksAgainstSalesController implements Initializable {
    ...
    private void loadBarChartData(String month,String year, String loadType){
          try {
            ...
            barChart.setTitle("Comparison between various "
                    + " drugs on packs sales of " + month + " | " + year);
            ...
            while(rs1.next()){
                ...
                while(rs2.next()){
                   ...
                   while(rs3.next()){
                        ...
                        String salesQuery = "SELECT SUM(charge),transNo FROM sales WHERE id ='" + 
                                packId +"' and item ='Pack' GROUP BY id;";
                        ResultSet rs4 = Query.getDataQuery(salesQuery);
                        if(rs4.next()){
                         ...
                         Platform.runLater(new Runnable() {
                             ...
                             public void run() {
                                if(onView.equalsIgnoreCase("Bar")){
                                     ...
                                     + "\\Documents\\Aloe\\Charts\\" +"Drug names against packs sales bar chart"+ ".pdf";
                                          convertToPdf(printedBarChart,1920,1080,fileName);
                                 }else{
                                    String fileName = System.getProperty("user.home") 
                                    + "\\Documents\\Aloe\\Charts\\" +"Drug names against packs sales pie chart"+ ".pdf";
    ...
    private void btnVLAction(ActionEvent event) {
        ...
        window.loadSmallWindow("/aloe/view/manager/charts/drugPacksAgainstSalesLarge.fxml", onView 
            ...
            while(rs1.next()){
                ...
                while(rs2.next()){
                   ...
                   while(rs3.next()){
                        String packId = rs3.getString(1);
                        String salesQuery = "SELECT SUM(charge),transNo FROM sales WHERE id ='" + 
                                packId +"' and item ='Pack' GROUP BY id;"; //Fetching sum of charge and transNo to be used to fetch transaction date
                        ResultSet rs4 = Query.getDataQuery(salesQuery);
                        if(rs4.next()){
                            if(loadType.equalsIgnoreCase("Partial")){
                                int transNo = rs4.getInt(2);
                                ...
                                if(rs5.next()){
                                     ...
                                     if(transDate.getMonth().name().equalsIgnoreCase(month) 
                                             && (transDate.getYear()+"").equalsIgnoreCase(year) ){
                                          totalCharge = totalCharge+ rs4.getDouble(1);
                                     }
                                }
                           }else{
                                totalCharge = totalCharge+ rs4.getDouble(1);
